# This script scrapes house transactions for Staunton, VA
...
for PID in range(1,12409):
    
    print(PID)
    
    try:
        
        VGSURL = 'https://gis.vgsi.com/StauntonVA/Parcel.aspx?pid=' + str(PID)
        ...
        if results_add == None:
            ...
            for i in range(1,len(sales)):
                ...
                try:
                    ...
                    occupancy.append(float(ugh[15].text))
                ...
                try:
                    ...
                    exwall2.append(float(ugh[19].text))
                ...
                try:
                    ...
                    fireplace.append(float(ugh[69].text))
...
occupancy = pd.Series(occupancy, name = 'Occupancy')
...
sf_gar = pd.Series(sf_gar, name = 'Garage_SF')
...
Sales_df = pd.concat([pids, address, typ, buyer, seller, price, date, year, feet, style,
                      model, grade, stories, occupancy, exwall1, exwall2, roof, cover, inwall1, inwall2,
                      infloor1, infloor2, heat, fuel, ac, beds, full, half, extra_fix, rooms, foundation,
                      fireplace, sf_gar, sf_bsmt, use_code, descr, neighborhood, acres], axis = 1)

Sales_df = Sales_df.replace('\xa0', None)

Sales_df.to_csv('D:/accidental_effect/data/staunton.csv', encoding = 'utf8', index = False)
